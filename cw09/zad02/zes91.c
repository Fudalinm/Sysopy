...
void* producer(void* ptr){
    ...
    while(1){
        ...
        if(WI == 1){printf("Producent %lu: Czekam na wziecie semaforu\n ",pthread_self() );}
        if(0 != sem_wait(argument->sem)) {printf("Nie udalo sie zablokowac mutexa w producencie\n");}
        ...
        if(argument->GB->Strings[argument->GB->NextToUpload] != NULL){ //wtedy nie mamy gdzie wpisac
            ...
            if(0 != sem_post(argument->sem)){printf("Nie udalo sie odblokowac semaforu w producencie\n");}
            continue;
        }
        ...
        if(NULL == fgets(buf,512,InFD)){
            ...
            if(0 != sem_post(argument->sem)){printf("Nie udalo sie odblokowac mutexa w producencie\n");}
            raisProducerKill();
        }
        ...
        if(0 != sem_post(argument->sem)){printf("Nie udalo sie zablokowac mutexa w producencie\n");}
    }
}
...
void* consumer(void* ptr){
    ...
    while(1){
        ...
        if(0 != sem_wait(argument->sem)) {printf("Nie udalo sie zablokowac mutexa w kliencie\n");}
        ...
        if(argument->GB->Strings[argument->GB->NextToSearch] == NULL){
            ...
            if(0 != sem_post(argument->sem)){printf("Nie udalo sie odblokowac mutexa w kliencie\n");}
            raiseConsumerKill();
            continue;
        }
...
int main(int argc,char* argv[]) {
    ...
    sem_t sem;
    if(sem_init(&sem,0,1) != 0){printf("Nie udalo sie stworzyc semafora\n");exit(1);}
